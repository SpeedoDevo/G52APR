...
public class RequestHandler implements IRequestHandler {
	...
	//booleans necessary for logging purposes
	...
	//get the requested file for respose logging
	public String getFile() {
		return error ? "" : reqTokens[1].replaceFirst("^/", "");
	}
	...
	private byte[] handleGet() {
		...
		try {
			...
			if (headers.containsKey("If-Modified-Since")) {
				...
				out = ServeProvider.getInstance().httpGETconditional(reqTokens[1],ifModifiedSince);
			} else {
				out = ServeProvider.getInstance().httpGet(reqTokens[1]);
			}
	...
	private byte[] handleHead() {
		byte[] out;
		try {
			out = ServeProvider.getInstance().httpHEAD(reqTokens[1]);
		} catch (HTTPFileNotFoundException e) {
			out = (NOT_FOUND + errorHeaders()).getBytes();
		} catch (HTTPRuntimeException e) {
			//never happens
		...
		try {
			...
			if (headers.containsKey("Content-Length")) {
				...
				if (expectedLength == body.length) {
					out = ServeProvider.getInstance().httpPOST(reqTokens[1],body);
				} //else stays bad req
			} //else stays bad req
		} catch (HTTPFileNotFoundException e) {
			out = (NOT_FOUND + errorHeaders()).getBytes();
	...
	private byte[] handleSimple() {
		...
		if(!reqTokens[0].equals("GET") || !isCorrectURI(reqTokens[1])) {
			...
			try {
				isSimple = true;
				out = ServeProvider.getInstance().simpleGet(reqTokens[1]);
				fallback = false;
			} catch (HTTPFileNotFoundException e) {
				out = (NOT_FOUND + errorHeaders()).getBytes();
			} catch (HTTPRuntimeException e) {
	...
	private int indexOf(byte[] data, byte[] pattern) {
		...
		int j = 0;
		// ~if (data.length == 0) to avoid literals
		if (data.length == j) {
			return -1;
		}
...
//	private void logReq() {
//		TransactionRegister logger = Registers.getRegister("req")
//			.collect("file",reqTokens[2].replaceFirst("^/", ""))
//			.collect("reqLine",lines[0]);
//		if(headers.containsKey("Date")) {
//			logger.collect("date",headers.get("Date"));
//		} else {
//			logger.collect("date",DateUtils.formatDate(new Date()));
//		}		
//	}
