...
public class ConnectionHandler implements Runnable {
	...
	public void run() {
		...
		try {
			...
			//make new log
			new RequestTransaction(req);
			RequestHandler requestHandler = new RequestHandler();
			byte[] res = requestHandler.processRequest(req);
			// send response
			out.write(res);
			//get the requested file from RequestHandler as it's not deducible from the bare response
			String file = requestHandler.getFile();
			//check whether it was a simple request or if there was an error fall back to full response
			if (requestHandler.isSimple() && !requestHandler.simpleFallback()) {
				new SimpleResponseTransaction(file);
			} else {
				new ResponseTransaction(res,file);
			}
